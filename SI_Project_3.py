...
def get_user_tweets(phrase):
	...
	if unique_identifier in CACHE_DICTION: # if it is...
		...
		public_tweets = api.user_timeline(phrase) # get it from the internet
...
for x in umich_tweets:
	for each in x['entities']['user_mentions']:
		userinfo = api.get_user(each['screen_name'])
	...
	tweet_id = x['id']
	text = x['text']
	user_posted = x['id_str']
	time_posted = x['created_at']
	retweets = x['retweet_count']
	cur.execute(statement2, (tweet_id, text, user_posted, time_posted, retweets))
	conn.commit()
...
screen_names = cur.execute(query).fetchall()
screen_names = [tup[0] for tup in screen_names]
...
query = 'SELECT screen_name, description FROM Users INNER JOIN Tweets ON Tweets.retweets WHERE retweets> 50'
...
for word in descriptions_fav_users:
	for match in re.findall("\S", word):
